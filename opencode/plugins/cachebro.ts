 ...
 *
 * Differences from buzinas/opencode-cachebro:
 *   - Imports from published "cachebro" entry point, not the deep source path
 *   - Adds file.edited hook to immediately invalidate cache when the agent writes
 *   - Adds file.watcher.updated hook for external changes
 *   - Cleans up watcher via session.deleted instead of process signal handlers
 *
          ...
          for (const p of args.paths) {
            try {
              ...
              if (result.cached && result.linesChanged === 0) {
                ...
                parts.push(`=== ${p} [${result.linesChanged} lines changed of ${result.totalLines}] ===\n${result.diff}`)
              } else {
                parts.push(`=== ${p} ===\n${result.content}`)
              }
            } catch (e: any) {
