...
class BashAnalyzer:
        ...
        for i, line in enumerate(self.lines, 1):
            ...
            if "${" in line and not ("{#" in line or "/#" in line or "%" in line):
                if "$" in line and '"$' not in line and "'" not in line:
                    self.issues["critical"].append(f"L{i}: unquoted variable expansion")
            if "eval" in line:
    ...
    def _format_results(self):
        ...
        if self.stats["forks"] > 5:
            out.append("⚠ Excessive forks ⇒ use bash builtins")
        out.append("")
        for category in ["critical", "performance", "optimization", "standards"]:
            if self.issues[category]:
                ...
                if len(self.issues[category]) > 15:
                    out.append(f"  ... +{len(self.issues[category]) - 15} more")
                out.append("")
        return "\n".join(out)
