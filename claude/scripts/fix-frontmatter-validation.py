...
def fix_frontmatter_field(
    frontmatter: str, field: str, fixer, filename: str = None
) -> str:
...
def process_file(filepath: Path, dry_run: bool = False) -> dict:
    ...
    if "description:" in frontmatter:
        ...
        if new_frontmatter != frontmatter:
            ...
            new_match = re.search(
                r"^description:\s*(.+)$", new_frontmatter, re.MULTILINE
            )
            if old_match and new_match:
                old_len = len(old_match.group(1).strip())
                new_len = len(new_match.group(1).strip())
                result["changes"].append(
                    f"description: {old_len} chars -> {new_len} chars"
                )
            frontmatter = new_frontmatter
